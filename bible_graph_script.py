#! /bin/python
"""Module to generate a Graph for data about the bible."""
# ---------------------------------------------------------------------------------------
...
class Graph(object):
    """Class of the actual Graph that is going to be generated."""
                ...
                if move:
                    ...
                    if new_rectangle[1][1] + y_distance < self.user_space_dim[1] - self.border:
                        new_rectangle = ((new_rectangle[0][0], new_rectangle[0][1] +
                                          y_distance), (new_rectangle[1][0],
                                                        new_rectangle[1][1] + y_distance))
                        restart = True
                        break
                    else:
...
class SVGFile(Graph):
    ...
    def __init__(self, filename, width, height, start_year, end_year, border, space):
        ...
        Graph.__init__(self, width, height, start_year, end_year, border, space)
...
def read_config(config_file_path):
    ...
    cli_parser = argparse.ArgumentParser(description=
                                         '''A script to create a graph of the events and persons
                                         of the bible.''')
